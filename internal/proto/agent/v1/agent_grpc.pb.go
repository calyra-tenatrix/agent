...
const (
	AgentService_AgentChannel_FullMethodName = "/agent.v1.AgentService/AgentChannel"
...
type AgentServiceClient interface {
	...
	AgentChannel(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[AgentMessage, BackendMessage], error)
...
func (c *agentServiceClient) AgentChannel(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[AgentMessage, BackendMessage], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &AgentService_ServiceDesc.Streams[0], AgentService_AgentChannel_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
...
type AgentService_AgentChannelClient = grpc.BidiStreamingClient[AgentMessage, BackendMessage]
...
type AgentServiceServer interface {
	...
	// Results/Events flow: Agent -> Backend
	// Health checks: Backend requests via stream, Agent responds via stream
	AgentChannel(grpc.BidiStreamingServer[AgentMessage, BackendMessage]) error
	// Periodic heartbeat for basic liveness monitoring (separate from main channel)
...
func (UnimplementedAgentServiceServer) AgentChannel(grpc.BidiStreamingServer[AgentMessage, BackendMessage]) error {
...
func _AgentService_AgentChannel_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(AgentServiceServer).AgentChannel(&grpc.GenericServerStream[AgentMessage, BackendMessage]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AgentService_AgentChannelServer = grpc.BidiStreamingServer[AgentMessage, BackendMessage]
	...
	Streams: []grpc.StreamDesc{
		{
			...
			Handler:       _AgentService_AgentChannel_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
	},
